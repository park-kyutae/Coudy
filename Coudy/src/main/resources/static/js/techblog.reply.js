$(function(){
	...
	function selectList(pageNum){
		...
		$.ajax({
			url:'listReply.do',
			type:'post',
			data:{pageNum:pageNum,tech_num:$('#tech_num').val()},
			dataType:'json',
			cache:false,
			timeout:30000,
	...
	$('#re_form').submit(function(event){
		if($('#tech_re_content').val().trim()==''){
			alert('내용을 입력하세요');
			$('#tech_re_content').val('').focus();
			return false;
		}
	...
	$(document).on('keyup','textarea',function(){
		...
		if(inputLength>300){//300자를 넘어선 경우
			...
			remain += '/300';
			if($(this).attr('id')=='tech_re_content'){
				//등록 폼 글자수
	...
	$(document).on('click','.modify-btn',function(){
		//댓글 글번호
		let tech_re_num = $(this).attr('data-num');
		//댓글 내용
